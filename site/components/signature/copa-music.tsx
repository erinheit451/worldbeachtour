...
export default function CopaMusic({ jobimImage }: { jobimImage?: SectionImage }) {
  return (
    <Section id="music" className="bg-volcanic-900 text-volcanic-50" dark>
      ...
      <div className="grid gap-10 lg:grid-cols-[1.6fr_1fr] items-start">
        <Prose dark>
          ...
          <p>
            Bossa nova left Brazil in 1962 when Antônio Carlos Jobim played Carnegie Hall, and
            returned as something adopted into the world's DNA — the film <em>Breathless</em>, Stan
            Getz's saxophone, the Pixies' "Here Comes Your Man." Copa's stake in the sound is its
            birthplace and its first venues. Ipanema's stake is its fame.
          </p>
          <p>
        ...
        {jobimImage && (
          <figure className="overflow-hidden rounded-xl bg-volcanic-800">
            <div className="aspect-[3/4] overflow-hidden">
              <img
                src={jobimImage.url}
                alt={jobimImage.title}
                className="w-full h-full object-cover"
              />
            </div>
            <figcaption className="px-4 py-3">
              <div className="font-display text-lg text-white">Antônio Carlos Jobim</div>
              <div className="text-[11px] text-volcanic-400 italic mt-1">
                {jobimImage.title} · {jobimImage.license}
              </div>
            </figcaption>
          </figure>
        )}
