...
from src.enrich.etopo_bathymetry import (
    _offshore_point, _compute_slope_and_flag, enrich_etopo_bathymetry,
)
...
def test_enrich_raises_when_etopo_missing():
    """Without ETOPO NetCDF, the pipeline fails loudly with a helpful message."""
    import sqlite3
    conn = sqlite3.connect(":memory:")
    conn.row_factory = sqlite3.Row
    with pytest.raises((FileNotFoundError, RuntimeError)) as ei:
        enrich_etopo_bathymetry(conn, path=Path("data/etopo/NOPE.nc"))
    assert "ETOPO" in str(ei.value) or "xarray" in str(ei.value)
